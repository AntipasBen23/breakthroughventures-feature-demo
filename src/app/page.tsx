...
export default function LiveDemoDayDashboard() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Randomly add a new interest every 10 seconds
      if (Math.random() > 0.7) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-white p-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10">
          <div className="text-sm text-gray-400 mb-1">Total Startups</div>
          <div className="text-3xl font-bold">{startups.length}</div>
        </div>
        <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10">
          <div className="text-sm text-gray-400 mb-1">Live Interests</div>
          <div className="text-3xl font-bold text-green-400">{interests.length}</div>
        </div>
        <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10">
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10">
          <div className="text-sm text-gray-400 mb-1">Investors Attending</div>
          <div className="text-3xl font-bold text-blue-400">{mockInvestors.length}</div>
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${stats.heat === 'hot' ? 'border-red-500/50 shadow-red-500/20' : ''}
                ${stats.heat === 'warm' ? 'border-orange-500/50 shadow-orange-500/20' : ''}
                ${stats.heat === 'cool' ? 'border-white/10' : ''}
              `}
            >
              {/* Heat Indicator */}
              {stats.heat === 'hot' && (
                <div className="absolute top-4 right-4 flex items-center gap-1 bg-red-500/20 text-red-400 text-xs px-2 py-1 rounded-full">
                  🔥 HOT
                </div>
              )}
              {stats.heat === 'warm' && (
                <div className="absolute top-4 right-4 flex items-center gap-1 bg-orange-500/20 text-orange-400 text-xs px-2 py-1 rounded-full">
                  📈 Warm
                </div>
              )}
      ...
      {selectedStartup && (
        ...
        >
          <div
            className="bg-slate-900 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-8 border border-white/10"
